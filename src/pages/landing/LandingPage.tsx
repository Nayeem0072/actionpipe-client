...
export function LandingPage() {
  ...
  const handleLogin = () => {
    loginWithRedirect({ authorizationParams: { connection: AUTH0_GOOGLE_CONNECTION } })
  }

  const handleSignUp = () => {
    loginWithRedirect({
      authorizationParams: { connection: AUTH0_GOOGLE_CONNECTION, screen_hint: 'signup' },
    })
  }
          ...
          <div className="header-actions">
            {!isLoading && (
              isAuthenticated && user ? (
                ...
                <>
                  <button type="button" className="btn-header btn-header-ghost" onClick={handleLogin}>Log in</button>
                  <button type="button" className="btn-header" onClick={handleSignUp}>Sign Up Now</button>
                </>
              )
            )}
          </div>
      ...
      <div className="hero-wrap">
        ...
        <section className="hero">
          ...
          <p className="hero-login">
            Already have an account? <button type="button" className="hero-login-btn" onClick={handleLogin}>Log in</button>
          </p>
